...
| Collapsing the rubric to one score | A high mean can hide a zero on causal validity | Ten dimensions; `verdict_from_profile` fails on that zero | `tests/test_rubric.py::test_zero_causal_validity_fails_despite_high_mean` | Weights if a single composite is required |
